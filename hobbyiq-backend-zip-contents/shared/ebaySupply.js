...
exports.getEbaySupplySnapshot = getEbaySupplySnapshot;
// Simulated eBay supply data service
// In production, replace with real eBay API or inventory DB
async function getEbaySupplySnapshot(player, cardName, parallel) {
    ...
    if (trend === "Falling")
        signal = "Tightening";
    else if (trend === "Rising" && baseListings > 25)
        signal = "Flooded";
    else if (trend === "Rising")
        signal = "Expanding";
    else if (trend === "Flat" && baseListings < 10)
        signal = "Tightening";
    return {
