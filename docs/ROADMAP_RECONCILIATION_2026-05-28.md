...
| Cosmos `hobbyiq-comps-centralus` regional endpoint at ~21-32% failure rate | **Investigated twice** — first characterization 2026-05-24 (`44e3884`) as upsert payload defect was wrong; PR #113 (`81f5c7b`) shipped a defensive id-validation guard against the wrong cause. Inverted 2026-05-26 (`3852e62`): the failures are cross-partition QUERY rejections (`getPlayerScoreByName` + `getTopPlayersByScore`), not upserts. **Current empirical state 2026-05-28**: 32% failure rate on DailyIQ-path queries (449 calls / 24h, 144 HTTP 400). Inversion is correct; deferral is NOT — traffic resumed, fix is ACTIVE in W1 as **CF-PLAYERTRENDS-QUERY-FAILURE**. The earlier "uninvestigated, no user-facing impact" framing in this row was wrong on both counts. | Active user-facing impact: ~32% of DailyIQ player-score lookups return null silently, degrading the daily brief + top-players surfaces. PR #113 stays as defensive coverage of a non-problem (don't remove). |
