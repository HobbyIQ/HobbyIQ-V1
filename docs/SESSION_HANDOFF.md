...
- **CF-CARDHEDGE-DECOMMISSION-FULL** (NEW, HIGH backlog, captured 2026-05-29 PM after CF-UNIFIED-SEARCH-AND-CERT W5-Windows ship) — next backend workstream after W5-iOS. W5-Windows migrated the user-facing endpoints (`/api/compiq/cardsearch` now calls the unified dispatcher; `/api/compiq/search-list` deleted entirely). CardHedge is still live for `/api/compiq/price-by-id` (takes `cardHedgeCardId`) and for `cardhedge.client.ts` itself + the `fn-cardhedge-comps` Azure Function. Scope: (1) migrate `/api/compiq/price-by-id` from CardHedge to Cardsight; (2) delete `cardhedge.client.ts` entirely; (3) disable `fn-cardhedge-comps` Azure Function; (4) remove `CARD_HEDGE_API_KEY` and CH env vars from `HobbyIQ3` App Settings; (5) cancel CardHedge subscription (business action, post-code-removal); (6) update `copilot-instructions.md` to remove CardHedge references; (7) rename or accept `cardHedgeCardId` as permanent column name on PortfolioHolding (data migration decision); (8) update tests/fixtures/docs that reference CardHedge; (9) deeper code-reference cleanup beyond W5-Windows Phase 1 enumeration. This is the original 2026-05-21 roadmap's "Phase 3 CH decommission" that was absorbed into v1. W5-Windows handled the user-facing endpoint migration; this CF handles the rest. Scope estimate: ~4-6h given unknown extent of CH integration; real Phase 1 grep for full CH footprint required before scoping fix. NOT a W5-Windows expansion. Separate CF, separate ship. **Sub-CF status (per D5 split locked 2026-05-30):** Sub-CF #1 CF-PRICE-BY-ID-MIGRATION SHIPPED at [5640084](https://github.com/HobbyIQ/HobbyIQ-V1/commit/5640084). Sub-CF #2 CF-FN-COMPS-MIGRATION PAUSED — see separate backlog entry below. Sub-CF #3 CF-CARDHEDGE-NAMING-CLEANUP not started.
- **CF-FN-COMPS-MIGRATION-SUB-2A-PAUSED** (NEW, HIGH backlog, paused 2026-05-30 evening for strategic review per Drew's Option 2 decision) — second sub-CF of CF-CARDHEDGE-DECOMMISSION-FULL Phase 2 per D5 split. Sub-2a Phase α implementation COMPLETE (Python Cardsight client + fn-cardhedge-comps migration with best-of-top-5 query strategy + `{player} {year}` fallback; 30/30 tests green) but dual gate FAILED on both criteria: coverage 10/18 vs ≥15 required, deviation 12.64% vs ≤10% required. **Findings surface a strategic question, not a tactical iteration:** the ±10% deviation gate from D8 was designed for "same vendor, same card, market drift" — empirical evidence shows the actual situation is **cross-vendor canonical-card-divergence** (CardHedge and Cardsight resolve different canonical cards for the same player query, those cards have different sale patterns, and the resulting compsMomentum signals legitimately disagree). 5 of 8 high-deviation cases have OPPOSITE signal direction (Vlad Jr, Witt Jr, Misiorowski, Julio Rodriguez at 30-41%; Aaron Judge at 12.64%). 2 players (Elly De La Cruz, Cooper Bonemer) lose signal entirely due to Cardsight catalog name-resolution gaps. **Three legitimate paths (Section 12.3 of design doc):** Path A — accept Cardsight as authoritative; ship migration with directionally-different signals for ~5 players; "everything is Cardsight" framing fully realized. Path B — revise D2 to Option C; defer migration; CardHedge subscription stays active for compsMomentum indefinitely. Path C — hybrid per-player canonical-card mapping table; manual curation burden. D2 was locked as Option A before these findings; future-Drew evaluates whether D2 stands or revises. Migration code is COMPLETE and tested but NOT DEPLOYED. Full findings + artifacts catalog in [`docs/phase0/fn_comps_migration_phase1_2026-05-30.md`](phase0/fn_comps_migration_phase1_2026-05-30.md) Sections 11-12. Sub-CFs #2b (fn-nightly-comp-prefetch + cosmos_floor.py + cleanup) ON HOLD pending Sub-2a strategic resolution.
