...
| **CF-FETCH-SIGNAL-FLOOR-TELEMETRY** | ⚠️ PARTIAL shipped | 9297957 (PR #124), deployed. Code paths shipped + 12/12 tests pass + URL sanitization (`?code=` not leaked) + graceful fallback preserved. **But:** post-deploy verification showed `trackDependency` calls NOT reaching App Insights `dependencies` table (2 `/predict` calls fired ~12 min apart; zero entries with name='signal_service' or 'price_floor_service'). Root cause hypothesis: `applicationinsights@^3.15.0` is the v3 OpenTelemetry-based rewrite; legacy `defaultClient.trackDependency()` shim doesn't reliably surface manual telemetry through the v3 pipeline, especially with the Azure App Service Agent (`ApplicationInsightsAgent_EXTENSION_VERSION=~3`) running concurrently. Auto-instrumented http/Cosmos still visible (agent picks them up directly). `trackException` paths exist and should fire on real failures (not testable today — all probes succeeded). The 2 HIGH-severity audit findings are now: failure-path partially addressed via trackException; success-path visibility deferred to CF-FETCH-TELEMETRY-V3-FIX. `/health` proactive check (PR #123) catches URL misconfig regardless. |
| **CF-FETCH-TELEMETRY-V3-FIX** | NEW, open | ~30-60 min investigation + variable fix scope. Investigate why `appInsights.defaultClient.trackDependency` calls in v3 SDK don't surface to App Insights `dependencies` table. Hypotheses to test: (a) sampling drops manual events, (b) v3 OTel shim requires explicit `flush()`, (c) App Service Agent contention suppresses npm SDK output, (d) v3 schema mismatch silently rejects records. Likely fix path: migrate the helper to `@opentelemetry/api` primitives (tracer.startSpan with span kind=CLIENT) instead of legacy trackDependency. Scaffolding from PR #124 reusable (`mcp-server/telemetry.ts` helper, test seam, sanitization logic, call-site wiring in pricing.ts). |
