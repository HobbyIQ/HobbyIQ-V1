...
| **CF-FETCH-TELEMETRY-COLUMN-MAPPING** | NEW, open, LOW | ~15-20 min. Post-PR-#125 verification surfaced: entries appear but `target` column shows the span name (`signal_service`) instead of hostname (`fn-compiq.azurewebsites.net`), `data` column is empty (should be `/api/signals`), `resultCode` = 0 (should be `200`/`400`). Azure Monitor's OTel exporter prefers the newer OTel semantic conventions over the deprecated ones used (`@opentelemetry/semantic-conventions` v1.x exports `SEMATTRS_HTTP_URL`/`SEMATTRS_PEER_SERVICE`/`SEMATTRS_HTTP_STATUS_CODE` — deprecated). Fix: switch to newer convention attributes: `url.full` (was `http.url`), `server.address` (was `peer.service`), `http.response.status_code` (was `http.status_code`). Single-file change to `mcp-server/telemetry.ts` plus matching test updates. Entries are already QUERYABLE today by name + success + duration — this CF is polish for full column-detail visibility, not a blocker. |
...
| **CF-EXPAND-TRACKED-PLAYERS-AND-RE-BACKTEST** | NEW, open, **recommended next** | ~30-60 min, ~$0.75. Per-grade diagnostic's load-bearing recommendation. Set `COMPIQ_TRACKED_PLAYERS` env var on `fn-compiq` to all 10 cohort players (currently only 5 tracked by default; ~$0 cost — env var change). Wait one aggregator cycle (2hr) for new players' first aggregated.json. Re-run N=15×5 backtest. Result determines whether subsequent work is CF-PHASE4B-PROMPT-AUDIT (Option B), CF-AGGREGATOR-PER-GRADE (Option A), CF-SIGNAL-CREDENTIAL-REPAIR (degraded signal repair), or just CF-BACKTEST-DETERMINISTIC (noise reduction). Closes the 40% no-signal cohort contamination revealed by the per-grade diagnostic. |
