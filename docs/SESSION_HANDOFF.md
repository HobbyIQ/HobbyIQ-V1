...
- **CF-CARDSIGHT-PRICING-BULK** (NEW, LOW backlog, captured 2026-05-29 during empirical follow-up Appendix A2.6) — evaluate `get_card_pricing_bulk` (1-100 card batch endpoint) for sibling-pool / candidate-pool computation paths where per-card pricing fan-out is sequential today. Discovered during 2026-05-29 empirical MCP tool enumeration ([`docs/phase0/cardsight_published_sdk_2026-05-29.md`](phase0/cardsight_published_sdk_2026-05-29.md) Appendix A2). Not urgent — current scale works at v1 volume — but 500/1000/5000 tier load patterns may make per-card pricing fan-out binding. Scope: identify call sites in HobbyIQ where multiple Cardsight cardIds are priced sequentially (likely `peerPoolBuilder.ts`, `compiqEstimate.service.ts` sibling-pool path), evaluate whether per-call latency saving × call-site frequency justifies the integration cost. Implementation per site = small refactor wrapping the bulk endpoint behind an iterator/batcher; not architectural. NOT in v1 scope; open at 500-tier launch-readiness assessment.
...
Three resolution paths for W5 to evaluate explicitly at kickoff (paths i-iii surfaced at W4 deferral; path iv was added 2026-05-29 morning by the published-SDK investigation and **REMOVED** 2026-05-29 late session per empirical follow-up — see below). Pointer: full sharpening at [`docs/phase0/cardsight_published_sdk_2026-05-29.md`](phase0/cardsight_published_sdk_2026-05-29.md) Appendix A.

  (i) **Add `image_url` + `variant` to `UnifiedSearchResponse`,** populated from `/catalog/cards/{id}` detail. **VARIANT GAP EMPIRICALLY SOLVED 2026-05-29** (Appendix A1): detail endpoint returns `parallels: Array<{ id, name, numberedTo? }>` cleanly — exact match for our existing `CardsightParallel` type from W3. Plus `attributes[]` field as bonus enrichment. **IMAGE GAP STILL REAL** — no CDN-redirect shortcut (Appendix A2.2 confirmed `get_card_image({ format: 'json' })` returns base64 bytes, not URL). Operational mitigation strategy required: lazy-fetch on row visibility, top-N priority fetch, parallelize detail+image per card, or accept the latency budget against the existing 24h cache.
  ...
  ~~(iv) `ai.query()` / `autocomplete.cards()` as alternative search endpoint.~~ — **REMOVED 2026-05-29 late session**. Empirically resolved NEGATIVE per Appendix A2.4: the MCP server's own tool definition documents `ai_query` as a server-side AI assistant returning synthesized natural-language answers, NOT structured `CardIdentity[]`. Verbatim from tool description: *"Don't use `ai_query` for known specific lookups — `get_card`, `get_card_pricing`, `search_cards` are faster and cheaper."* `autocomplete_cards` similarly is not richer than `search_cards`. Path (iv) is NOT viable as a picker backend.

**Sharpened W5 picker question (post-empirical-follow-up):** Given path (i) cleanly solves the variant gap via per-hit detail fetch (~2.5-6.3s fan-out at observed Cardsight ~8 req/s rate limit) but the image gap requires a separate per-hit fetch (another ~2.5-6.3s) with no CDN-redirect shortcut available — **which image-fetch mitigation strategy fits the iOS picker UX best**: lazy-fetch on row visibility, top-N priority fetch, parallelize detail+image per card, or accept the latency budget against the existing 24h cache? This is a concrete design question with an empirical answer space, not an open architectural question. W5 kickoff's empirical pre-flight collapses to a single optional ~5-10 card image-latency measurement (sized in Appendix A6 "still unverified") — the heavy lifting is done.

**Transport architectural question (α) vs (β) — added 2026-05-29 by published-SDK investigation.** Choice Y locked the iOS consumer shape as `UnifiedSearchResponse`; transport remained open. The two transport implementations are:

- (α) iOS calls Cardsight directly via the published Swift SDK; backend dispatcher bypassed
- (β) iOS keeps calling `/api/search/cards` (W3 shipped at `d5a3169`); backend dispatcher mediates

**Default-state framing — important:** (β) is what's shipped today. The decision is "stay on (β) or rebuild as (α)," not a greenfield "pick one." Default-on-no-decision is (β). Lean (NOT a decision): stay on (β) — the W3 dispatcher abstraction is load-bearing for v1.5 grader pluggability, server-only API key is load-bearing, Phase 4a cache interceptability is load-bearing. (α) would erode all three. See [investigation doc §6](phase0/cardsight_published_sdk_2026-05-29.md) for full tradeoffs.

**Choice Y is preserved across all four paths and both transport choices.** Only field provenance + transport differ; consumer shape is the constant.
