...
- **CF-CARDHEDGE-NAMING-CLEANUP** (NEW, LOW backlog, captured 2026-05-29 post-CF-CARDHEDGE-HARD-CUTOVER) — rename the `cardHedgeCardId` column on `PortfolioHolding` + the `CardHedgeCard` / `CardHedgeSale` types in `cardsight.router.ts` to vendor-neutral names. Currently kept as legacy names for backward compat post-cutover (acceptable single-user pre-launch); rename when convenient. Sub-CF #3 of the original CF-CARDHEDGE-DECOMMISSION-FULL scope. Scope estimate: ~1-2h backend + small Cosmos data migration for the column rename.
- **CF-CARDHEDGE-RESIDUAL-DOC-SWEEP** (NEW, LOW backlog, captured 2026-05-29 PM via Phase 1.5 grep during CF-CARDHEDGE-DOCS-CLEANUP) — second-pass cleanup of CardHedge references outside the three CF-CARDHEDGE-DOCS-CLEANUP scoped files. **In scope:** (1) `compiq-functions/fn-signal-aggregator/function.py:19-22` — comment block referencing deleted `fn-cardhedge-comps`; one-line update noting hard-cutover + greenfield CF status. (2) `PR-A1-DESCRIPTION.md` at repo root — historical PR body; consider archiving to `docs/phase0/`. (3) `docs/security/SECRET_ROTATIONS.md` — annotate CARD_HEDGE_API_KEY entry with "decommissioned 2026-05-29 at 10ad39d". (4) `backend/harness/README.md` + `backend/harness/tier1/_helpers.ts` + `backend/harness/tier1/nonBaseball.test.ts` + `backend/docs/phase-c-checklist.md` + `backend/docs/parallels-reference-schema.md` — minor harness/schema doc updates. **Explicitly OUT of scope:** (a) all `docs/phase0/*.md` investigations (frozen audit-trail by design); (b) all `backend/docs/investigations/*.md` (same); (c) `backend/docs/decisions/ADR-*.md` (historical decision records by nature); (d) `docs/HOBBYIQ_ROADMAP_2026Q2_Q3.md` + `docs/ROADMAP_RECONCILIATION_2026-05-28.md` (superseded predecessors with explicit supersedes pointer from active roadmap); (e) post-cutover breadcrumb comments in test files + architectural lineage comments in cardsight.router.ts / compiqEstimate.service.ts / unifiedSearch/* (load-bearing for future maintainers; ~60 comments). **Scope estimate:** ~30-45 min. Two dead probe scripts calling `api.cardhedger.com` (`backend/scripts/parallels-2b-i-skenes-sample.ts` + `backend/scripts/parallels-2b-iii-bowman-chrome-2024-paginate.ts`) DELETED in CF-CARDHEDGE-DOCS-CLEANUP Phase 2 (not part of this CF).
