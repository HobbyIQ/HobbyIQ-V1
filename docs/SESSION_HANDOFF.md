...
1. **CF-COMPSMOMENTUM-GREENFIELD-CARDSIGHT Phase 3b** — verify 02:00 UTC nightly run:
   - All 10 tracked players produce fresh `compsMomentum.json` blobs (timestamps after 2026-05-31T02:00 UTC)
   - Signal aggregator reads blobs cleanly
   - No `fn-comps-momentum` exceptions in 24h window

2. **CF-CARDSIGHT-IDENTIFY-INTEGRATION happy-path smoke** (Mac-side; slab image more naturally available there):
...
If either verification surfaces issue: HALT, debug, potential rollback.
...
- **CF-CARDSIGHT-COLLECTION-SURFACE-INVESTIGATION** (NEW, MEDIUM backlog, captured 2026-05-30 during CF-CARDSIGHT-IDENTIFY-INTEGRATION Phase 1 agent investigation) — Cardsight's published surface includes `collection.*` entity endpoints (per Appendix A2.2 of the 2026-05-29 SDK deep-read): collection list, collection card add/remove, collection-card pricing aggregation. Strategic question: should HobbyIQ's PortfolioHolding storage federate with Cardsight collections, OR remain independent first-class storage with Cardsight as data provider only? **Pros of federation:** Cardsight handles collection-card identity, parallel attribution, grade tagging; per-card pricing aggregation handled vendor-side. **Cons of federation:** vendor lock-in for primary user data; pricing-logic independence lost; multi-user collection-sharing dependent on Cardsight's auth model; data-portability question on Cardsight contract termination. Phase 1 investigation: enumerate Cardsight collection endpoints + auth model + data-ownership terms; compare against HobbyIQ's existing PortfolioHolding write path + analytics/forecasting layer; surface federation vs independence decision for product roadmap review. NOT in v1 scope; relevant when launch volume justifies the architectural decision. Scope estimate: ~2-3h Phase 1 investigation; further phases gated on decision outcome.
