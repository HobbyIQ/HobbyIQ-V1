...
Four resolution paths for W5 to evaluate explicitly at kickoff (paths i-iii surfaced at W4 deferral; path iv surfaced 2026-05-29 by the Cardsight published-SDK investigation at [`docs/phase0/cardsight_published_sdk_2026-05-29.md`](phase0/cardsight_published_sdk_2026-05-29.md)):

  (i) **Add `image_url` + `variant` to `UnifiedSearchResponse`,** populated from `/catalog/cards/{id}` detail. Solve N+1 latency via parallel fetch + rate-limit awareness (Cardsight ~8 req/s observed during cardsight-cert-investigation), OR via batched detail call if Cardsight supports it. Verification of batch support is the first concrete W5 task. Updated 2026-05-29: SDK investigation confirmed no batched-detail or `expand=` option is documented; mitigations to evaluate at W5 kickoff are lazy-fetch on row visibility, top-N priority fetch, or empirical check whether `images.getCard()` returns a CDN-redirect URL that collapses the N+1 cost.
  ...
  (iv) **NEW — `ai.query()` / `autocomplete.cards()` as alternative search endpoint.** SDK investigation confirmed these exist as separate endpoints from `catalog.search()`. Path (iv) viability depends on `ai.query` actually returning identify-card-shape with `parallel` inlined (Cardsight's identify-card endpoint DOES inline parallel; whether `ai.query` follows the same shape is unknown until empirically verified). **Binary outcome:** if YES, the variant problem collapses entirely at the search-result layer and only image fetch remains. If NO, fall back to paths (i)/(ii)/(iii). Cost of resolving the binary: ~1 hour of read-only SDK calls at W5 kickoff before locking a path. **Worth doing first.**
