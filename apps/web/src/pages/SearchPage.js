...
const MultimodalInput_1 = __importDefault(require("../components/MultimodalInput"));
const EXAMPLES = [
    "Is Brady Ebel good?",
...
function SearchPage() {
    ...
    const [submittedQuery, setSubmittedQuery] = (0, react_1.useState)(null);
    const [feedbackSent, setFeedbackSent] = (0, react_1.useState)(null);
    ...
    const sendFeedback = async (feedback) => {
        ...
        try {
            await fetch(`${api_1.API_BASE_URL}/api/feedback`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
    ...
    const handleSaveToPortfolio = async () => {
        ...
        try {
            const res = await fetch(`${api_1.API_BASE_URL}/api/portfolio/user-uuid/cards`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    return (<div style={{ maxWidth: 700, margin: '40px auto', fontFamily: 'sans-serif' }}>
      <MultimodalInput_1.default value={input} onChange={setInput} onSubmit={handleSearch} loading={loading}/>
      <div style={{ display: 'flex', gap: 10, marginBottom: 32, flexWrap: 'wrap', justifyContent: 'center', width: '100%', maxWidth: 520 }}>
        {EXAMPLES.map(example => (<button key={example} type="button" style={{
                background: '#f0f4ff',
            ...
            }} onClick={() => handleExample(example)} disabled={loading}>
            {example}
          </button>))}
      </div>
      {error && (<div style={{ color: "#b71c1c", background: "#fff0f0", borderRadius: 8, padding: "10px 16px", marginBottom: 16, fontSize: 15, border: "1.5px solid #ffd6d6" }}>
          {error}
        </div>)}
      {submittedQuery && (<div style={{ marginBottom: 8, color: "#888", fontSize: 15, width: '100%', maxWidth: 520, textAlign: 'left' }}>
          <strong>Query:</strong> {submittedQuery}
        </div>)}
      {loading && (<div style={{ color: '#1976d2', fontSize: 16, margin: '24px 0', textAlign: 'center', width: '100%' }}>
      ...
      {response && !loading && (<div style={{ width: '100%', maxWidth: 520, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', marginTop: 0 }}>
          <div style={{
                background: "#fff",
                borderRadius: 18,
            ...
            <div style={{ marginBottom: 14, fontSize: 17, color: '#222', lineHeight: 1.6 }}>
              ...
              {response.result && (<div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
                  {response.result.recommendation && (<span style={{ background: '#e3f2fd', color: '#1976d2', fontWeight: 700, borderRadius: 6, padding: '2px 10px' }}>
                      {response.result.recommendation.toUpperCase()}
                    </span>)}
                  {response.result.FMV && (<span style={{ background: '#fffde7', color: '#bfa100', fontWeight: 700, borderRadius: 6, padding: '2px 10px' }}>
                      FMV: ${response.result.FMV}
                    </span>)}
                  {response.result.confidence && (<span style={{ background: '#e8f5e9', color: '#388e3c', fontWeight: 700, borderRadius: 6, padding: '2px 10px' }}>
                      Confidence: {response.result.confidence}
                    </span>)}
                  {response.result.compRange && (<span style={{ background: '#f3e5f5', color: '#7b1fa2', fontWeight: 700, borderRadius: 6, padding: '2px 10px' }}>
                      Comp Range: {response.result.compRange}
                    </span>)}
                  {response.result.urgency && (<span style={{ background: '#ffebee', color: '#b71c1c', fontWeight: 700, borderRadius: 6, padding: '2px 10px' }}>
                      Urgency: {response.result.urgency}
                    </span>)}
                  {response.result.trend && (<span style={{ background: '#e1f5fe', color: '#0288d1', fontWeight: 700, borderRadius: 6, padding: '2px 10px' }}>
                      Trend: {response.result.trend}
                    </span>)}
                </div>)}
              <span>{response.summary}</span>
            </div>
            {response.bullets && response.bullets.length > 0 && (<ul style={{
                    margin: 0,
                    paddingLeft: 18,
                    fontSize: 15,
                    color: '#444',
                ...
                {response.bullets.map((b, i) => (<li key={i} style={{ marginBottom: 2 }}>{b}</li>))}
              </ul>)}
            {/* Save to Portfolio button */}
            <div style={{ marginTop: 18, width: '100%' }}>
              <button onClick={handleSaveToPortfolio} disabled={saveStatus === 'saving'} style={{
                background: '#1976d2',
                color: '#fff',
                border: 'none',
                borderRadius: 8,
              ...
              {saveStatus === 'error' && (<span style={{ color: '#b71c1c', marginLeft: 10 }}>Could not save. Try again.</span>)}
              {saveStatus === 'success' && (<span style={{ color: '#388e3c', marginLeft: 10 }}>Saved!</span>)}
            </div>
            {response.nextActions && response.nextActions.length > 0 && (<div style={{ marginTop: 18, background: '#f0f4ff', borderRadius: 8, padding: '10px 16px', color: '#1976d2', fontSize: 15, fontWeight: 500, width: '100%' }}>
                ...
                <ul style={{ margin: 0, paddingLeft: 18, fontSize: 15, color: '#1976d2', lineHeight: 1.5, listStyle: 'circle' }}>
                  {response.nextActions.map((a, i) => (<li key={i} style={{ marginBottom: 2 }}>{a}</li>))}
                </ul>
              </div>)}
            {/* Highlight key values for all intents */}
            {response.result && Object.keys(response.result).length > 0 && (<div style={{ marginTop: 16, width: '100%' }}>
                {/* ...existing intent-specific UI... */}
                {/* Feedback buttons */}
                <div style={{ marginTop: 18, display: 'flex', gap: 12, alignItems: 'center' }}>
                  <span style={{ fontSize: 15, color: '#888', marginRight: 8 }}>Was this helpful?</span>
                  <button onClick={() => sendFeedback("helpful")} disabled={feedbackSent === "helpful"} style={{
                  ...
                  <button onClick={() => sendFeedback("not_helpful")} disabled={feedbackSent === "not_helpful"} style={{
                  ...
                  {feedbackSent && (<span style={{ marginLeft: 10, color: '#388e3c', fontSize: 15, fontWeight: 500 }}>
