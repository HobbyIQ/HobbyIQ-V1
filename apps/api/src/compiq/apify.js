...
exports.fetchSoldListingsFromApify = fetchSoldListingsFromApify;
// Build Apify API URL for eBay sold data (dataset endpoint)
function buildApifyDatasetUrl(params) {
...
async function fetchSoldListingsFromApify(params) {
    ...
    if (!res.ok) {
        if (res.status === 404)
            throw new Error("DATASET_ERROR: Apify dataset not found or not ready");
    ...
    const normalized = filtered.map(item => ({
        title: item.title,
        soldPrice: parsePrice(item.price || item.soldPrice),
        soldDate: item.date ? new Date(item.date).toISOString() : (item.soldDate ? new Date(item.soldDate).toISOString() : ""),
        rawTitle: item.title,
