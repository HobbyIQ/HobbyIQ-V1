...
exports.addPortfolioCard = addPortfolioCard;
exports.updatePortfolioCard = updatePortfolioCard;
exports.deletePortfolioCard = deletePortfolioCard;
exports.refreshPortfolio = refreshPortfolio;
exports.getPortfolioSummary = getPortfolioSummary;
const client_1 = require("@prisma/client");
const summary_1 = require("./summary");
const service_1 = require("../engines/hobbyiq/service");
const service_2 = require("../alerts/service");
...
async function createPortfolio(input) {
    if (!input.userId || !input.name)
        return { success: false, error: "Missing userId or name" };
...
async function getPortfolio(portfolioId) {
    if (!portfolioId)
        return { success: false, error: "Missing portfolioId" };
    const portfolio = await prisma.portfolio.findUnique({ where: { id: portfolioId } });
    if (!portfolio)
        return { success: false, error: "Portfolio not found" };
    return { success: true, data: { ...portfolio, createdAt: portfolio.createdAt.toISOString(), updatedAt: portfolio.updatedAt.toISOString() } };
}
async function addPortfolioCard(userId, portfolioId, cardInput) {
    if (!userId || !portfolioId)
        return { success: false, error: "Missing userId or portfolioId" };
    ...
    const card = await prisma.portfolioCard.create({
        data: {
            ...
            gainLossDollar: 0,
            gainLossPercent: 0,
            currentRecommendation: client_1.Recommendation.HOLD,
...
async function updatePortfolioCard(cardId, updates) {
    if (!cardId)
        ...
        try {
            ...
            await prisma.portfolioCard.update({
                ...
                data: {
                    ...
                    currentRecommendation: analysis.decisionOutput?.recommendation || client_1.Recommendation.HOLD,
            ...
            await prisma.cardAnalysisSnapshot.create({
                data: {
                    ...
                    recommendation: analysis.decisionOutput?.recommendation || client_1.Recommendation.HOLD,
            ...
            // Evaluate alerts
            await (0, service_2.createAlertIfNotDuplicate)({
                userId: card.userId,
...
async function getPortfolioSummary(portfolioId) {
    const cards = await prisma.portfolioCard.findMany({ where: { portfolioId } });
    const summary = (0, summary_1.buildPortfolioSummary)(cards.map(card => ({
        ...card,
        cardNumber: card.cardNumber === null ? undefined : card.cardNumber,
        serialNumber: card.serialNumber === null ? undefined : card.serialNumber,
        printRun: card.printRun === null ? undefined : card.printRun,
