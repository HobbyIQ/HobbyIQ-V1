...
exports.getEbaySoldStats = getEbaySoldStats;
...
async function fetchEbaySoldRaw(query) {
    // Example: Apify actor HTTP API
    const APIFY_ACTOR_URL = process.env.APIFY_EBAY_SOLD_URL;
    const APIFY_TOKEN = process.env.APIFY_TOKEN;
    if (!APIFY_ACTOR_URL || !APIFY_TOKEN)
        throw new Error('eBay sold API not configured');
    ...
    try {
        ...
        if (!res.ok)
            throw new Error('Failed to fetch eBay sold data');
...
async function getEbaySoldStats(query) {
