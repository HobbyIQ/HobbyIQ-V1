...
exports.PARALLELS = void 0;
exports.detectParallel = detectParallel;
exports.normalizeComp = normalizeComp;
...
function extractPlayerAndSet(title) {
...
function detectGrader(title) {
    for (const grader of GRADERS) {
        if (new RegExp(`\\b${grader}\\b`, "i").test(title))
            return grader.toUpperCase();
    }
    // Heuristic: "raw", "ungraded", "authentic", "hand cut", "altered"
    if (/\b(raw|ungraded|authentic|hand ?cut|altered)\b/i.test(title))
        return "RAW";
    return null;
}
...
function detectGrade(title) {
    ...
    if (/\bGEM ?MINT\b/.test(text))
        return "GEM MINT";
    if (/\bPRISTINE\b/.test(text))
        return "PRISTINE";
    if (/\bNM[- ]?MT\b/.test(text))
        return "NM-MT";
    if (/\bEX[- ]?MT\b/.test(text))
        return "EX-MT";
    if (/\bEX\b/.test(text))
        return "EX";
    if (/\bVG\b/.test(text))
...
function detectParallel(title) {
    for (const p of exports.PARALLELS) {
...
function computeMatchScore(comp, query) {
    let score = 0;
    if (query) {
...
function normalizeComps(rawItems, query) {
    return rawItems
        .map(raw => {
