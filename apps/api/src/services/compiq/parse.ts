import { CompIQRequest } from "./types";
...
export function parseCompIQInput(input: CompIQRequest): ParsedCompInput {
  ...
  let normalizedParallel: string | null = null;
  ...
  // Card type logic
  if (isAuto) cardType = "Auto";
  else cardType = "Base";

  // Product family logic (simple example)
  if (cardSet) {
    if (/prizm/i.test(cardSet)) productFamily = "Prizm";
    else if (/optic/i.test(cardSet)) productFamily = "Optic";
    else if (/select/i.test(cardSet)) productFamily = "Select";
    else productFamily = null;
  }

  // Warnings for missing fields
  if (!player) warnings.push("Player not detected");
  if (!cardSet) warnings.push("Set not detected");
  if (!parallel) warnings.push("Parallel not detected");
