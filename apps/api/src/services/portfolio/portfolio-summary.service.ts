...
export class PortfolioSummaryService {
  computeSummary(userId: string, positions: PortfolioPosition[]): PortfolioSummary {
    ...
    positions.forEach(p => {
      ...
      if (p.unrealizedGainLossPct != null) {
        if (p.unrealizedGainLossPct > 40) trimCount++;
        else if (p.unrealizedGainLossPct < -15) watchCount++;
        else if (p.unrealizedGainLossPct > 10) holdCount++;
        else if (p.unrealizedGainLossPct < 0) buyMoreCount++;
        else holdCount++;
      } else holdCount++;
    });
    return {
