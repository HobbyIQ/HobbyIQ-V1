...
import { CompIQResult, PortfolioEvaluationResult } from "../../types/hobbyiq";

export type AlertType = "SELL_ALERT" | "BUY_ALERT" | "PANIC_ALERT" | "WATCH_ALERT";

export interface HobbyIQAlert {
  type: AlertType;
  player: string;
  message: string;
...
export function generateCompIQAlerts(comp: CompIQResult): HobbyIQAlert[] {
  const alerts: HobbyIQAlert[] = [];
...
export function generatePortfolioIQAlerts(portfolio: PortfolioEvaluationResult): HobbyIQAlert[] {
  const alerts: HobbyIQAlert[] = [];
