...
const service_2 = require("../engines/decision/service");
const service_3 = require("../engines/selliq/service");
// --- Integration points: inject real services here ---
// PlayerIQ now uses the real engine for playeriq intent
const playerIQService = {
    async getPlayerReport(player, query) {
        try {
            const perf = await (0, playerPerformance_1.getPlayerPerformance)(player);
            if (!perf || !perf.stats) {
                return {
...
function getCompIQCacheKey(params) {
...
const compIQService = {
    async getCardComps(card, query, parsed) {
        try {
            ...
            // Fetch and compute
            const comps = await (0, service_1.getCompIQComps)(params);
            // Handle dataset error
            if (comps._fetchError && comps._fetchErrorType === "DATASET_ERROR") {
                return {
                    success: false,
            ...
            // Handle fetch error
            if (comps._fetchError) {
                return {
                    title: `Value Guidance: ${card}`,
                    summary: `Could not fetch comp data for ${card}: ${comps._fetchErrorMessage}`,
                    bullets: [
                        `An error occurred while fetching comp data: ${comps._fetchErrorMessage}`,
            ...
            if (comps._noComps) {
                return {
                    success: true,
                    compsFound: false,
                    reason: comps._noCompsReason,
                    suggestion: comps._noCompsSuggestion,
                    data: []
                };
            }
            // Normalized/pricing output
            const pricing = comps._pricing || {};
            return {
                success: true,
                compsFound: true,
...
const buySellDecisionService = {
    async getDecision(card, query) {
        try {
            ...
            const dec = (0, service_2.runDecisionEngine)(decInput);
            const sell = (0, service_3.runSellIQ)({
                currentFMV: 120,
                riskAdjustedFMV: 115,
                quickExitFMV: 110,
                compTrendPercent: 8,
...
const comps_1 = require("../services/comps");
const compareService = {
    async getComparison(left, right, query) {
        try {
            // Fetch comps for both sides
            const [leftComps, rightComps] = await Promise.all([
                (0, comps_1.getComps)(left),
                (0, comps_1.getComps)(right)
            ]);
            // Helper to summarize comps
            function summarizeComps(comps, label) {
                if (!comps || comps.length === 0)
                    return `${label}: No recent sales found.`;
...
async function handleSearch(req) {
    ...
    const intent = (0, classifier_1.classifyIntent)(req.query);
    const playerOrCard = parsed.player || (0, intentClassifier_1.extractPlayerOrCard)(req.query);
    const q = req.query;
    ...
    switch (intent) {
        ...
        case 'buy_sell_decision': {
            ...
            response = {
                ...
                confidenceLabel: decisionResult.confidenceLabel ?? '',
                confidenceExplanation: decisionResult.confidenceExplanation ?? ''
            };
            break;
        }
