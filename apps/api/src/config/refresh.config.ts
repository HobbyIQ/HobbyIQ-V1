export interface RefreshConfig {
  hotCardTtlMinutes: number;
  mediumCardTtlMinutes: number;
  coldCardTtlMinutes: number;
  playerSummaryTtlMinutes: number;
  staleServeAllowedMinutes: number;
  weightedMedianChangePctThreshold: number;
  activeSupplyChangePctThreshold: number;
  demandRatioChangePctThreshold: number;
  confidenceDeltaThreshold: number;
  maxRetryCount: number;
}

export const refreshConfig: RefreshConfig = {
