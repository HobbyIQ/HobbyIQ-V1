...
class ApiUser(HttpUser):
    ...
    host = "hobbyiq-andjgvhgfbhfcuhv.azurewebsites.net"
    ...
    def get(self):
        ...
        with self.client.get(
            ...
            if response.status_code in [200]:
                response.success()
                if self.ENABLE_LOGGING:
                    print("[Locust] GET api/health succeeded")
            else:
                msg = f"GET api/health failed with status {response.status_code}, response: {response.text}"
