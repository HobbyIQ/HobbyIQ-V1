...
function calculatePricing(medianPrice, salesCount, activeListings) {
...
async function fetchEbaySoldData(query) {
  ...
  try {
    ...
    console.warn('[compiqSearch] Apify fetch failed:', err.message);
    return [];
  }
}
...
async function searchAndPrice(query) {
  ...
  const activeListings = 0; // not available from sold feed alone

  const { estimatedFMV, scarcityScore, confidence } = calculatePricing(
    medianPrice,
    salesCount,
    activeListings,
  );
