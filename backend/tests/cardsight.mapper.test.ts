...
// CF-CARDSIGHT-RESOLVER-TIFFANY — lookupReleaseName with parallel + year
// overrides. Pure function tests, no Cardsight mocks needed.
//
// Background: Cardsight catalogs Tiffany parallels as separate cardIds with
// distinct setNames ("1987 Topps Traded Tiffany Baseball" vs "1987 Topps
// Traded Baseball"). The resolver was greedy-picking base via pricing-probe
// records-based selection. Dictionary override: when parallel="Tiffany" and
// product+year matches an enumerated set, return the dedicated Tiffany
// setName so the resolver targets the right cardId.
//
// Enumeration: 14 entries (1984-1991 Topps + 1986/87/89/91 Topps Traded +
// 1996/97 Fleer) per Cardsight catalog probes 2026-05-27.
describe("CF-CARDSIGHT-RESOLVER-TIFFANY — lookupReleaseName parallel + year overrides", () => {
  describe("Tiffany override fires for enumerated product+year pairs", () => {
    it("Topps Traded 1987 + Tiffany → Topps Traded Tiffany setName", () => {
      expect(lookupReleaseName("Topps Traded", "Tiffany", 1987))
        .toBe("1987 Topps Traded Tiffany Baseball");
    });

    it("Topps 1987 + Tiffany → Topps Tiffany base setName (not Traded)", () => {
      // Note: this is the user's stored data case for the Maddux holding.
      // Returns the BASE Topps Tiffany set ("1987 Topps Tiffany Baseball")
      // not the Traded set. Maddux's RC isn't in this set, so without data
      // correction (product="Topps Traded") the resolver won't find Maddux
      // records here. Data correction is part of the same CF.
      expect(lookupReleaseName("Topps", "Tiffany", 1987))
        .toBe("1987 Topps Tiffany Baseball");
    });

    it("Topps 1984 + Tiffany → earliest enumerated Topps Tiffany year", () => {
      expect(lookupReleaseName("Topps", "Tiffany", 1984))
        .toBe("1984 Topps Tiffany Baseball");
    });

    it("Topps 1991 + Tiffany → latest enumerated Topps Tiffany year", () => {
      expect(lookupReleaseName("Topps", "Tiffany", 1991))
        .toBe("1991 Topps Tiffany Baseball");
    });

    it("Fleer 1996 + Tiffany → Fleer Tiffany setName", () => {
      expect(lookupReleaseName("Fleer", "Tiffany", 1996))
        .toBe("1996 Fleer Tiffany Baseball");
    });

    it("case-insensitive product + parallel match", () => {
      expect(lookupReleaseName("TOPPS TRADED", "TIFFANY", 1987))
        .toBe("1987 Topps Traded Tiffany Baseball");
      expect(lookupReleaseName("topps traded", "tiffany", 1987))
        .toBe("1987 Topps Traded Tiffany Baseball");
    ...
    it("whitespace trimmed on product + parallel", () => {
      expect(lookupReleaseName("  Topps Traded  ", "  Tiffany  ", 1987))
        .toBe("1987 Topps Traded Tiffany Baseball");
    });
  });

  describe("Tiffany override does NOT fire — falls through to base dictionary", () => {
    it("year outside enumerated range (1983) → null (no Tiffany match) AND no base dictionary key 'topps' → null", () => {
      expect(lookupReleaseName("Topps", "Tiffany", 1983)).toBeNull();
    });

    it("year outside enumerated range (1992 Topps) → null", () => {
      expect(lookupReleaseName("Topps", "Tiffany", 1992)).toBeNull();
    });

    it("Topps Traded 1984 + Tiffany → null (gap in Topps Traded Tiffany enumeration)", () => {
      // Cardsight catalog has Topps Traded Tiffany for 1986/87/89/91 but
      // NOT 1984/85/88/90 — this confirms the gap behavior.
      expect(lookupReleaseName("Topps Traded", "Tiffany", 1984)).toBeNull();
    });

    it("Topps Traded 1988 + Tiffany → null (gap year)", () => {
      expect(lookupReleaseName("Topps Traded", "Tiffany", 1988)).toBeNull();
    });

    it("different parallel (Glossy) does NOT trigger Tiffany override", () => {
      // Falls through to base dictionary; "topps" not in base dict → null
      expect(lookupReleaseName("Topps", "Glossy", 1987)).toBeNull();
    });

    it("parallel undefined → falls through to base dictionary (existing behavior)", () => {
      expect(lookupReleaseName("Topps Update")).toBe("Topps Update");
      expect(lookupReleaseName("Bowman Chrome")).toBe("Bowman Chrome");
    });

    it("parallel='Tiffany' but year missing → falls through to base dictionary", () => {
      // Without year, can't disambiguate which Tiffany set. Fall through.
      expect(lookupReleaseName("Topps Update", "Tiffany")).toBe("Topps Update");
    });

    it("year=null → falls through to base dictionary", () => {
      expect(lookupReleaseName("Topps Update", "Tiffany", null))
        .toBe("Topps Update");
    });

    it("non-Tiffany product (Topps Chrome) + Tiffany parallel → base dictionary still resolves", () => {
      // Edge case: "Topps Chrome + Tiffany" — the override only handles
      // base Topps + Topps Traded + Fleer (no Topps Chrome Tiffany). Should
      // fall through to base dictionary and return "Topps Chrome".
      expect(lookupReleaseName("Topps Chrome", "Tiffany", 2024))
        .toBe("Topps Chrome");
    });
  });

  describe("Backward compatibility — single-argument calls preserve existing behavior", () => {
    it("lookupReleaseName('Topps Update') → 'Topps Update' (unchanged)", () => {
      expect(lookupReleaseName("Topps Update")).toBe("Topps Update");
    });

    it("lookupReleaseName('Bowman Chrome') → 'Bowman Chrome' (unchanged)", () => {
      expect(lookupReleaseName("Bowman Chrome")).toBe("Bowman Chrome");
    });

    it("lookupReleaseName('unknown product') → null (unchanged)", () => {
      expect(lookupReleaseName("Random Brand Name")).toBeNull();
    });

    it("lookupReleaseName('') → null (unchanged)", () => {
      expect(lookupReleaseName("")).toBeNull();
    });
  });
});

// CF-CARDSIGHT-RESOLVER-TIFFANY — integration: resolver picks Tiffany
// cardId when query input has parallel + year that hits the dictionary.
describe("CF-CARDSIGHT-RESOLVER-TIFFANY — resolveCardId picks Tiffany cardId", () => {
  const ms = (m: typeof cs) => ({
    searchCatalog: m.searchCatalog as unknown as ReturnType<typeof vi.fn>,
    getCardDetail: m.getCardDetail as unknown as ReturnType<typeof vi.fn>,
    getPricing: m.getPricing as unknown as ReturnType<typeof vi.fn>,
  });

  beforeEach(() => {
    ms(cs).searchCatalog.mockReset();
    ms(cs).getCardDetail.mockReset();
    ms(cs).getPricing.mockReset();
  });

  it("Maddux Topps Traded Tiffany 1987 → resolves to Tiffany cardId via release-name exact-match", async () => {
    // Search query becomes "Greg Maddux 1987 Topps Traded Tiffany Baseball".
    // Cardsight returns both Tiffany + base entries — release-name exact-match
    // filter narrows to the Tiffany one before pricing-probe.
    ms(cs).searchCatalog.mockResolvedValue([
      catalog("tiffany-id", "1987 Topps Traded Tiffany Baseball"),
      catalog("base-id", "1987 Topps Traded Baseball"),
    ]);
    // After release filter narrows to single candidate, no detail or pricing
    // probes are needed for the cardId pick. Parallel resolution at the end
    // still runs getCardDetail, but only AFTER cardId selection.
    ms(cs).getCardDetail.mockResolvedValue(
      detail("tiffany-id", "70T", []),
    );
    ...
    expect(r.cardId).toBe("tiffany-id");
    expect(r.matchConfidence).toBe("exact");
  });

  it("parallel='Tiffany' without year → falls through; no Tiffany override fires", async () => {
    // No Tiffany dictionary hit (no year). releaseName comes from base dict
    // ("Topps Update" stays "Topps Update"). Resolver behaves as before.
    ms(cs).searchCatalog.mockResolvedValue([
      catalog("update-id", "Topps Update"),
    ]);
    ms(cs).getCardDetail.mockResolvedValue(
      detail("update-id", "US175", []),
    );
    ...
    const r = await resolveCardId({
      ...
      product: "Topps Update",
      parallel: "Tiffany",
      // cardYear intentionally omitted
    });

    expect(r.cardId).toBe("update-id");
  });
});
