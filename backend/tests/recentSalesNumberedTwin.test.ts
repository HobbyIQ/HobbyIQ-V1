...
describe("GET /cards/:cardId/recent-sales -- an un-numbered id reads itself AND its numbered twin", () => {
  it("count 35, requestedCardId = the un-numbered id, resolvedCardId = …:num-499, poolCardIds = both, identityKind numbered-twin", async () => {
    ...
    // Scoped to STARTSWITH against the UNION PARAMETERS specifically (R71,
    // 2026-09-19 round 2 added a legitimate STARTSWITH(c.hobbyiqCardId,
    // 'hiq:<sport>:') inside the park-carve-out clause, against a hardcoded
    // sport literal, never against @cid/@cid1) -- see
    // readCompsByCardIdNumberedTwin.test.ts for the same note.
    expect(h.queries[0].query).not.toContain("STARTSWITH(c.hobbyiqCardId, @cid");
    expect(h.queries[0].query).not.toContain("STARTSWITH(c.cardId, @cid");
  });
