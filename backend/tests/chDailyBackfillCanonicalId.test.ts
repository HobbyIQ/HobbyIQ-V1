...
const { findResidentTwin } = backfillScript;
...
// + skipped-other + failed. Pinned as a standalone arithmetic check
// mirroring the banner's own equation, since the banner print itself is
// not a unit under test.
// ---------------------------------------------------------------------
describe("reconcile arithmetic — read = written + skipped-twin-resident + skipped-other + failed", () => {
  it("balances when every row lands in exactly one bucket", () => {
    ...
    const failed = 1;
    expect(written + skippedTwinResident + skippedOther + failed).toBe(processed);
  });
  ...
  it("flags an imbalance (a row landed in zero or two buckets) rather than hiding it", () => {
    ...
    expect(written + skippedTwinResident + skippedOther + failed).not.toBe(processed);
  });
});
