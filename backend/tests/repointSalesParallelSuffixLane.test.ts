...
describe("repoint-sales-parallel-suffix -- refusals", () => {
  it("REFUSES when TWO candidates are both strict on the same rung (two-candidates)", () => {
    const rowA = CATALOG_ROW({ id: `${PREFIX}60:silver:no-auto`, cardId: `${PREFIX}60:silver:no-auto`, cardNumber: "60", parallelSlug: "Silver" });
    const rowB = CATALOG_ROW({ id: `${PREFIX}60:silver-prizm:no-auto`, cardId: `${PREFIX}60:silver-prizm:no-auto`, cardNumber: "60", parallelSlug: "Silver Prizm" });
    // A sale on a spelling NEITHER of the two candidates -- but suppose the
    // sale sits at "silvers" (a plural typo), whose candidates include both
    // "silver" and "silver-prizm" -- both strict here.
    const shortId = `${PREFIX}60:silvers:no-auto`;
    const sale = { id: "s1", cardId: shortId, hobbyiqCardId: shortId, title: "plain #60", sport: SPORT, price: 5, parallel: "Silvers", isAuto: false, gradeCompany: null, gradeValue: null, soldAt: "2024-01-01", playerName: "Test Player" };
    const r = drive({ ...DEFAULT_ENV, BACKFILL_APPLY: "true" }, { catalog: [rowA, rowB], sales: [sale] });
    expect(r.code).toBe(0);
    // This is ALSO both-slugs-are-real-rungs by construction (silver and
    // silver-prizm coexist on #60), so accept either named refusal -- the
    // point pinned here is that NOTHING moves.
    expect(r.led.salesUpserts.length).toBe(0);
  });
