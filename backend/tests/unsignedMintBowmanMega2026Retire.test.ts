 ...
 * verified always-auto product-years. Scope: baseball 2026 bowman-mega,
 * prefixes BMA-/RMA-.
 *
...
const list = JSON.parse(
  readFileSync(
    path.join(dataDir, "catalog-relocations", "2026-09-21-unsigned-mint-baseball-2026-bowman-mega.json"),
    "utf8",
  ),
);
...
describe("2026-09-21 unsigned-mint retire list: baseball 2026 bowman-mega", () => {
  ...
  it("has exactly 509 entries, all unique ids", () => {
    expect(entries).toHaveLength(509);
    expect(new Set(entries.map((e) => e.id)).size).toBe(509);
    expect(list.census.catalogRowsRetired).toBe(509);
  });
  ...
  it("every id is baseball/2026/bowman-mega, one of the two verified prefixes, and ends :no-auto (optionally with a print-run)", () => {
    const pattern = /^hiq:baseball:2026:bowman-mega:(bma|rma)-[a-z0-9-]+:[a-z0-9-]+:no-auto(:num-\d+)?$/;
    for (const e of entries) {
      expect(e.id, e.id).toMatch(pattern);
    }
  });

  it("every entry names a defective source and a non-defective :auto twin in its reason/evidence", () => {
    for (const e of entries) {
  ...
  it("the resident-sale exclusions are reported, not routed, and are NOT in the entries list", () => {
    expect(list.census.excludedForResidentSales).toBe(73);
    expect(list.census.salesResidentDetail).toHaveLength(73);
  ...
  it("the no-good-twin exclusions are tallied by reason, not listed", () => {
    expect(list.census.excludedNoGoodTwin).toBe(678);
    expect(list.census.excludedNoGoodTwinReasons["no twin row at :auto id"]).toBe(678);
    const listedIds = new Set(entries.map((e) => e.id));
