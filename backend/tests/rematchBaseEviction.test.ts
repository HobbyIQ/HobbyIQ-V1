...
describe("MUTATION CHECK: the protected guard on BASE-EVICTION is load-bearing", () => {
  ...
  it("removing the tier check from the subclass makes a PROTECTED row writable", () => {
    ...
    // The expression carries a fourth conjunct now (the derivation-defect
    // refusals, CF-A-DERIVATION-DEFECT-IS-NOT-A-RULING). The marker is the
    // WHOLE expression, so each mutation below still removes exactly one
    // conjunct and leaves the others standing.
    const marker = "writable: prov.tier === AUTO && contradicting.length === 0 && !family.qualifies && derivationRefused.length === 0,";
    expect(src).toContain(marker);
    expect(src.split(marker)).toHaveLength(2);
    const mutated = src.replace(marker, "writable: contradicting.length === 0 && !family.qualifies && derivationRefused.length === 0,");
...
describe("MUTATION CHECK: the axis gate on BASE-EVICTION is load-bearing", () => {
  ...
  it("removing the axis conjunct makes a contradicting row writable again", () => {
    ...
    expect(src.split(marker)).toHaveLength(2);
    const mutated = src.replace(marker, "writable: prov.tier === AUTO && !family.qualifies && derivationRefused.length === 0,");
    expect(mutated).not.toBe(src);
