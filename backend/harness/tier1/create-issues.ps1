# Create 4 GitHub issues for HobbyIQ-V1 using gh CLI
$gh = "C:\Program Files\GitHub CLI\gh.exe"
$repo = "HobbyIQ/HobbyIQ-V1"

# 1. Ensure labels exist (ignore errors if they already do)
$labels = @(
    @{name="bug"; color="d73a4a"; desc="Confirmed bug"},
    @{name="bug-suspected"; color="fbca04"; desc="Suspected bug, needs investigation"},
    @{name="compiq"; color="0e8a16"; desc="CompIQ pricing engine"},
    @{name="parser"; color="c5def5"; desc="Query parser"},
    @{name="grade-handling"; color="c5def5"; desc="Card grade handling"},
    @{name="vintage"; color="c5def5"; desc="Vintage cards"},
    @{name="comp-filtering"; color="c5def5"; desc="Comp pool filtering"},
    @{name="needs-investigation"; color="fbca04"; desc=""},
    @{name="endpoint-consistency"; color="c5def5"; desc="Cross-endpoint divergence"},
    @{name="similar-to-pr-4-hotfix"; color="c5def5"; desc=""}
)
foreach ($l in $labels) {
    & $gh label create $l.name --color $l.color --description $l.desc --repo $repo 2>$null
}

# 2. Create issues from body files
$bodyDir = "backend\harness\tier1\issue-bodies"
...
$issues = @(
    @{
        ...
        file   = "$bodyDir\01-psa-variant-mismatch.md"
        body = @'
## Description
...
'@
    ...
    @{
        ...
        file   = "$bodyDir\02-vintage-grade-tier.md"
        body = @'
## Description
...
'@
    ...
    @{
        ...
        file   = "$bodyDir\03-under-anchored.md"
        body = @'
## Description
...
'@
    ...
    @{
        ...
        file   = "$bodyDir\04-endpoint-divergence.md"
        body = @'
## Description
...
foreach ($it in $issues) {
    $it.body | Out-File -FilePath $it.file -Encoding utf8
}
...
foreach ($it in $issues) {
    Write-Host ""
    Write-Host "Creating: $($it.title)" -ForegroundColor Yellow
    & $gh issue create --repo $repo --title $it.title --body-file $it.file --label $it.labels
}
