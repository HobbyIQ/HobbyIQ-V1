...
const { measureProductCoverage, productsOf, resolveNewSource, coverageLine } = require("./lib/sourceCoverage.cjs");
...
async function main() {
  ...
  console.log(`[audit-source-coverage] READ-ONLY  old=${OLD_SOURCES.join(",")}  new=${NEW_SOURCE}  floor=${MIN_COVERAGE_PCT}%  limit=${LIMIT || "all"}\n`);
  ...
  let totOldRows = 0, totNewRows = 0, totKeys = 0, totExact = 0, totNorm = 0, atFloor = 0, belowFloor = 0, nothing = 0, done = 0;
  for (const p of scope) {
    const c = await measureProductCoverage(cat, retry, p, OLD_SOURCES, NEW_SOURCE);
    if (!c) continue;
    done++;
    totOldRows += c.oldRows; totNewRows += c.newRows; totKeys += c.keys; totExact += c.coveredExact; totNorm += c.coveredNorm;
    const ok = c.pctNorm >= MIN_COVERAGE_PCT;
    if (ok) atFloor++; else belowFloor++;
    if (c.newRows === 0) nothing++;
  ...
  console.log(`\nTOTAL (${f(done)} products${LIMIT ? `, largest ${LIMIT}` : ""}): old ${f(totOldRows)} rows / ${f(totKeys)} keys, new ${f(totNewRows)} rows, covered exact ${f(totExact)} (${totKeys ? Math.round((100 * totExact) / totKeys) : 0}%) normalised ${f(totNorm)} (${totKeys ? Math.round((100 * totNorm) / totKeys) : 0}%)`);
  console.log(`  products at or above the ${MIN_COVERAGE_PCT}% floor: ${f(atFloor)}   below (the retire keeps them): ${f(belowFloor)}   new source has nothing: ${f(nothing)}`);
}
