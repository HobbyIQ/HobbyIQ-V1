...
// -- THE CLOCK --------------------------------------------------------------
//
// CF-A-KILLED-JOB-CANNOT-REPORT-PROGRESS. This lane PATCHES the composite
// parallel identity onto sold_comps rows and declared no budget at all. Its own
// comment puts the corpus at 3M rows and BACKFILL_LIMIT caps the SCAN at
// 500,000 per pass -- but a row cap is not a clock: half a million patches at a
// throttled moment take however long they take, so before this the lane could
// only ever end by being KILLED at the ceiling with no marker, no reconcile and
// no finishLane line.
//
// THIS LANE IS SCAN-THEN-WRITE, and both phases are on the clock.
//
//   THE SCAN gathers the whole capped population into memory first
//   (`rows.push(...)` over every page) before a single patch is issued. Its
//   unit is one page of up to 5,000 rows.
//
//   THE WRITE unit is ONE PATCH -- a point write on (id, cardId) -- dispatched
//   through a CONCURRENCY-wide (default 32) worker pool. 90 seconds comfortably
//   exceeds one patch plus the pool's drain, and it is checked BEFORE the patch
...
// is made from the ROW IN HAND -- parseParallelComposite reads that row's own
// title, cardNumber, sport and setName and nothing else -- so a row the scan
// never reached is simply not in this pass's plan rather than a missing input
// to some other row's answer.
//
...
async function main() {
  ...
  // row's answer comes from its own fields). What is NOT safe is STARTING the
  ...
  stopped at the ${CLOCK.RUN_MINUTES}-minute budget -- `
      ...
      + ` continues from here`);
    console.log("  nothing was written. The scan query selects only rows whose composite is absent"
  ...
  stopped at the ${CLOCK.RUN_MINUTES}-minute budget -- `
    ...
    console.log("  the patch is IDEMPOTENT: the scan selects only rows whose composite is absent or"
