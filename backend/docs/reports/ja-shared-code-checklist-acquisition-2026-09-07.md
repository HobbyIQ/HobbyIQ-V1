...
## What the 8 + neo1–4 would mint — measured, report-only

Local read-only run of the committed vintage lane, 2026-09-07. **Nothing written to Cosmos, no lane
armed.** Every set lands on its ruled address, and none on an English key.

| JA id | year | ruled setKey | rows | setName |
|---|---:|---|---:|---|
| `PMCG1` | 1996 | `ja-base1` | **102** | Japanese Base Set |
| `PMCG2` | 1997 | `ja-base2` | **48** | Japanese Jungle |
| `PMCG3` | 1997 | `ja-base3` | **48** | Japanese Fossil |
| `PMCG5` | 1998 | `ja-gym1` | **96** | Japanese Gym Heroes |
| `PMCG6` | 1999 | `ja-gym2` | **98** | Japanese Gym Challenge |
| `neo1` | 2000 | `ja-neo1` | **96** | Japanese Neo Genesis |
| `neo2` | 2000 | `ja-neo2` | **57** | Japanese Neo Discovery |
| `neo3` | 2000 | `ja-neo3` | **57** | Japanese Neo Revelation |
| `E1` | 2001 | `ja-ecard1` | **128** | Japanese Expedition Base Set |
| `neo4` | 2001 | `ja-neo4` | **113** | Japanese Neo Destiny |
| `E2` | 2002 | `ja-ecard2` | **92** | Japanese Aquapolis |
| `E3` | 2002 | `ja-ecard3` | **90** | Japanese Skyridge |

```
  sets staged        12
  card rows        1025
  unnameable          0   <- every card carried a dexId; none guessed
  sets skipped        0
```

**323 of those rows are neo1–4**, which no dispatch could have reached at all until this change: the
manifest was generated by the same dropping filter and never listed them.

### Two things the run caught that a green count would have hidden

**`Japanese E1` was a row named after its own id.** The lane's `SET_EN` table had no entry for the
e-Card sets, so `E1` fell through to its bare id — the provenance loss the modern lane's manifest
comment describes. The names now come from the source-id map, which already had to know them.

**`PMCG4` must NOT follow this rule.** Rocket Gang's ruled key is `japanese-rocket-gang` (R4), where
the catalog already holds 65 tcgdex-ja rows plus 14 sales-attested. The old name-slug key reached it
*by accident*, via `normalizeSetKey(setName)`; deriving from the id would have keyed it `pmcg4` and
opened a **second pool for one set**. The R4 named key is stated explicitly and wins over any code
derivation — verified: PMCG4 still stages to `1997-japanese-rocket-gang-pokemon`, byte-identical to
before.

## THE ACQUISITION QUEUE — the 4 genuine BW-era asks

These are the only cells on this page that need a publisher we do not hold. Stated in the repo's
per-`year | setKey` cell format so a source survey can be run against them directly.

| year | setKey | JA product (alias) | EN twin | source held | candidate | status |
|---:|---|---|---|---|---|---|
| 2011 | `ja-bw2` | red-collection | `bw2` Emerging Powers | **none** | — | **OPEN — permissive JA source wanted** |
| 2012 | `ja-bw4` | dark-rush | `bw4` Next Destinies | **none** | — | **OPEN — permissive JA source wanted** |
| 2012 | `ja-bw7` | plasma-gale | `bw7` Boundaries Crossed | **none** | — | **OPEN — permissive JA source wanted** |
| 2013 | `ja-bw9` | megalo-cannon | `bw9` Plasma Freeze | **none** | — | **OPEN — permissive JA source wanted** |

**The ask is one source, not four.** All four are the same era from the same publisher, so a single
permissive 2011-13 Japanese BW checklist source closes every cell. tcgdex serves no set under `BW2`,
`BW4`, `BW7` or `BW9` — re-confirmed live 2026-09-07 — and the two obvious alternatives stay refused
on licence (see above), which is why these are an acquisition and not a lane change.

Each key **stands** as the correct address in the meantime. Under
CF-PRICE-ONLY-CHECKLIST-MATCHED-IDENTITIES these four pools do not price until a checklist lands,
which is not a regression: those rows do not price today either, they merely pool with the English
card, and absent beats wrong.
...
1. ~~The **8 HELD** cells want a lane change so the tcgdex JA vintage sets stage under their ruled
   `ja-<code>` key.~~ **DONE** — CF-THE-JAPANESE-SET-IS-REACHED-BY-ITS-JAPANESE-ID (2026-09-07).
   `scripts/lib/tcgdex-ja-source-id-map.cjs` states each JA source id's product identity with its
   evidence, and R5 spells the address; the vintage lane now derives its setKey from it and **writes
   it into the manifest**, where both the ingest and the driver's verification read it.
   `PCG1` is pinned as a fixture that MUST refuse, so the 4 DP cells stay open.
...
**Nothing in this report was applied.** No Cosmos write, no staging, no manifest change. The lane
and manifest changes referenced in item 1 landed separately, and are themselves report-only until a
runner dispatch arms them.
