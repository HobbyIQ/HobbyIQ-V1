...
export async function augmentCompsWithUserPool(
  ...
  // CF-USER-COMPS-PARALLEL-FILTER (Drew, 2026-07-18): when the caller
  // knows the holding's parallel, pass it here so the user-pool query
  // filters to same-parallel comps only. CH's card-search returns the
  // same cardId for every parallel sharing a cardNumber, so without
  // this filter a "Blue Refractor" holding's FMV gets diluted by Blue
...
export async function computeEstimate(
  ...
  if (!body.cardId && fetched.card === null && fetched.comps.length === 0) {
    // CF-PRODUCT-FAMILY-PROJECTION (2026-07-09, Drew — Owen Carey 2026
    // Bowman Black Sapphire): pure-math projection for whole-product-line
    // catalog gaps (Sapphire is the launch case — released but not yet
    // indexed by CH). Fires BEFORE the sibling rescue below because it's
    // more specific (targets known product-line gaps) and produces a
