...
export async function persistVendorSalesToPool(
  ...
  for (const row of rows) {
    ...
    //
    // (Drew, 2026-09-19.) The one seam, same reasoning R29 states just above:
    // sport, year, setKey and player are ALL in hand here, and rewriting
    // `setKey` to the insert's own registered product key BEFORE
    // `computeHobbyIqCardId` runs means the id it mints is the insert's card,
    // never the base card of that number. Lives here rather than inside
    // `hobbyIqCardId.service.ts` -- a declared derivation-stamp input -- for
    // the same reason `soldCompsStore.recordSoldComp` carries its own copy of
    // this pre-step: the change belongs to what gets WRITTEN, not to what
    // either stamp-input module derives in isolation.
    //
