...
export async function recordSoldComp(input: RecordSoldCompInput): Promise<RecordSoldCompResult> {
  ...
  //
  // (Drew, 2026-09-19.) An async-free pre-step, before slug derivation, so it
  // can rewrite `input.setName` to the insert's OWN product key -- the same
  // input both `deriveHobbyIqSlug` below and the stored `doc.setName` read --
  // rather than living inside `hobbyIqCardId.service.ts` /
  ...
  if (!insertPinCandidate.startsWith("hiq:")) {
    ...
    if (insertSport && String(insertSport).toLowerCase() !== "pokemon" && typeof input.cardYear === "number") {
      const insertSetKey = resolveSetKeyForSlug(insertSport, input.setName ?? "", input.cardYear);
      if (insertSetKey && insertSetKey !== "unknown") {
        ...
        if (insertMatches.length > 1) {
          ...
          if (only.registeredKey) {
            ...
            // insert's own id, not the base card's.
            input = { ...input, setName: only.registeredKey };
          } else {
