...
exports.compiqDecisionEngine = compiqDecisionEngine;
exports.compiqPricingEngine = compiqPricingEngine;
exports.getCompIQComps = getCompIQComps;
exports.calculateCompIQMarketStats = calculateCompIQMarketStats;
function runCompIQ(comps) {
    ...
    if (parallelLadder.base)
        baseRef = "base";
    else if (parallelLadder.refractor)
        baseRef = "refractor";
    const multipliers = {};
    if (baseRef) {
    ...
    if (!estimates.base) {
        const est = { raw: Math.round(median) };
        estimates.base = est;
    }
    ...
    const riskSignals = [];
    // 1. Price Trend
    const compsSortedByDate = [...validComps].filter(c => c.date).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
 ...
 */
function compiqDecisionEngine({ median, latestPrice, confidence, compTrend }) {
    let signal = "HOLD";
...
function compiqPricingEngine(comps) {
    ...
    const getSortedPrices = (arr) => arr.map(c => c.soldPrice).filter(p => typeof p === 'number' && p > 0).sort((a, b) => a - b);
    // Helper: median
    const medianOf = (arr) => {
        const n = arr.length;
        if (n === 0)
...
function filterOutliers(comps) {
    ...
    return comps.filter((c) => c.soldPrice >= min && c.soldPrice <= max);
}
...
async function getCompIQComps(params) {
    try {
        const results = await (0, apify_1.fetchSoldListingsFromApify)(params);
        ...
        const normalized = results.map(item => ({
            title: item.title,
            soldPrice: parseFloat((item.soldPrice || item.price || '').toString().replace(/[^\d.]/g, "")) || 0,
            soldDate: item.soldDate ? String(item.soldDate) : (item.date ? String(item.date) : ""),
...
function calculateCompIQMarketStats(comps) {
    if (!comps.length) {
        return {
            fmv: null,
            low: null,
    ...
    // 2. Recency weights
    const weights = filtered.map((c) => recencyWeight(c.soldDate));
    // 3. FMV (weighted median)
    const prices = filtered.map((c) => c.soldPrice);
    const fmv = weightedMedian(prices, weights);
    // 4. Range
    ...
    if (filtered.length > 2) {
        // x = days ago, y = price
        const xs = filtered.map((c) => (Date.now() - new Date(c.soldDate).getTime()) / (1000 * 60 * 60 * 24));
        const ys = prices;
        const wsum = weights.reduce((a, b) => a + b, 0);
        const xbar = xs.reduce((a, b, i) => a + b * weights[i], 0) / wsum;
        const ybar = ys.reduce((a, b, i) => a + b * weights[i], 0) / wsum;
        let num = 0, den = 0;
    ...
    const now = Date.now();
    const days = filtered.map((c) => (now - new Date(c.soldDate).getTime()) / (1000 * 60 * 60 * 24));
    const within30 = days.filter((d) => d <= 30).length;
    const liquidity = within30;
    // 7. Comp count
    ...
    const avgDays = days.reduce((a, b) => a + b, 0) / compCount;
    if (avgDays < 14)
        confidence += 30;
    else if (avgDays < 60)
        confidence += 15;
    // Variance: lower = higher confidence
    const mean = prices.reduce((a, b) => a + b, 0) / compCount;
    const variance = prices.reduce((a, b) => a + (b - mean) ** 2, 0) / compCount;
    if (variance < 0.1 * mean ** 2)
        confidence += 20;
    else if (variance < 0.2 * mean ** 2)
        confidence += 10;
    // Data completeness
    if (filtered.every((c) => c.soldDate && c.soldPrice && c.title))
        confidence += 10;
    if (confidence > 100)
        confidence = 100;
    // 9. Return
