...
export interface TrendResult {
  compCount: number;
  baseCompFmv: number | null;
  recentMedian: number | null;
  olderMedian: number | null;
  trendPct: number | null;
  trendDirection: "up" | "flat" | "down" | "insufficient_data";
  trendMultiplier: number;
  finalAdjustedFmv: number | null;
}
...
export function analyzeTrend(
  sales: CompSale[],
  opts?: { minComps?: number; recentCount?: number; round?: (n: number) => number }
): TrendResult {
  const minComps = opts?.minComps ?? 5;
  const recentCount = opts?.recentCount ?? 4;
  const round = opts?.round ?? ((n: number) => Math.round(n * 100) / 100);
  ...
  let trendDirection: "up" | "flat" | "down" | "insufficient_data" = "flat";
