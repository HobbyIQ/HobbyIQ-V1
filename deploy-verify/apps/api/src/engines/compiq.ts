...
import { getEbaySoldStats } from "../services/ebaySold";
...
export async function compiqEngine(req: UniversalSearchRequest): Promise<UniversalSearchResult> {
  ...
  let stats;
  try {
    stats = await getEbaySoldStats(req.query);
  } catch (e) {
  ...
  let confidence = 0;
  if (compCount === 0) confidence = 0;
  else if (compCount === 1) confidence = 0.2;
  else if (compCount < 4) confidence = 0.4;
  else if (compCount < 8) confidence = 0.6;
  else confidence = 0.8;
  ...
  if (compCount > 0) {
    ...
    if (daysAgo > 30) confidence *= 0.7;
    if (compRange && compRange.high && compRange.low && weightedMedianFMV && compRange.high - compRange.low > weightedMedianFMV * 0.5) confidence *= 0.7;
  }
  ...
  if (hasData) {
    ...
    if (keyNumbers.Range) why.push(`Comp range: ${keyNumbers.Range}`);
    if (keyNumbers.Trend) why.push(`Recency-weighted trend: ${keyNumbers.Trend}`);
    if (keyNumbers.Liquidity) why.push(`Liquidity: ${keyNumbers.Liquidity}`);
    why.push(`Sample size: ${compCount} recent sales.`);
    if (confidence < 0.5) why.push("Low confidence: few or messy comps, or wide price range.");
  } else {
