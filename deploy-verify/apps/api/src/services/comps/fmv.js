...
exports.calculateFmv = calculateFmv;
const exactMatch_1 = require("./exactMatch");
...
function scoreConfidence(comps, bucketLabel) {
    if (!comps.length)
        return { score: 0, compCount: 0, avgMatch: 0, avgRecency: 0, gradeConsistency: 0, parallelConsistency: 0 };
    ...
    if (!comps.every(c => (c.parallel || null) === firstParallel))
        parallelConsistency = 70;
...
function bucketByGrade(comps) {
    ...
    for (const c of comps) {
        ...
        if (!grade || grade === "RAW")
            ...
            buckets["PSA 9"]?.push?.(c);
        else if (grade === "PSA 10")
            buckets["PSA 10"]?.push?.(c);
        else
 ...
 */
function calculateFmv(comps, exactMatchOpts, opts) {
    ...
    if (exactMatchOpts) {
        usable = (0, exactMatch_1.filterExactMatches)(usable, exactMatchOpts);
    }
    if (!usable.length) {
        return {
            summary: {
    ...
    if (trimmed.length > 0 && Array.isArray(trimmed)) {
        ...
        if (first && typeof first.totalPrice === 'number')
            low = Math.round(first.totalPrice);
        if (last && typeof last.totalPrice === 'number')
            high = Math.round(last.totalPrice);
    }
    ...
    for (const label of ["RAW", "PSA 9", "PSA 10", "OTHER"]) {
        ...
        if (bucket && bucket.length) {
            ...
            if (bucketTrimmed.length > 0 && Array.isArray(bucketTrimmed)) {
                ...
                if (first && typeof first.totalPrice === 'number')
                    bucketLow = Math.round(first.totalPrice);
                if (last && typeof last.totalPrice === 'number')
                    bucketHigh = Math.round(last.totalPrice);
            }
    ...
    if (opts && opts.cardId && opts.playerId && opts.marketSegment && !opts.mockMode) {
        ...
        let weightsUsed = modelWeightProfileRepository_1.modelWeightProfileRepository.getActiveBySegment(opts.marketSegment);
        if (!weightsUsed) {
            // fallback: use any profile for segment
            const all = modelWeightProfileRepository_1.modelWeightProfileRepository.getBySegment(opts.marketSegment);
            if (all && all.length)
                weightsUsed = all[0];
        }
        pricingSnapshotRepository_1.pricingSnapshotRepository.add({
            cardId: opts.cardId,
