import { PlayerIQRequest, PlayerIQResponse, CardMarketSnapshot, TopGemRateCard } from "./types";
import { scorePlayerIQ } from "./scoring";
import { getRiskBand } from "./risk";
import { buildPlayerIQSummary } from "./summary";

function buildCardMarketSnapshot(player: string): CardMarketSnapshot {
...
function buildTopGemRateCards(player: string): TopGemRateCard[] {
...
export async function runPlayerIQ(input: PlayerIQRequest): Promise<PlayerIQResponse> {
  const warnings: string[] = [];
  if (!input.player) warnings.push("Player name is required");

  const scores = scorePlayerIQ(input);
  const risk = getRiskBand(scores);
  const summaryBlock = buildPlayerIQSummary(input, scores, risk);
  ...
  if (warnings.length) nextActions.push("Refine your search with a full player name");
  else nextActions.push("View comps", "Add to portfolio");
