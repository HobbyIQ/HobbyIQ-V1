...
interface Props {
  result: PlayerIQResponse;
}

const PlayerIQResultCard: React.FC<Props> = ({ result }) => {
  ...
  return (
    <div className="playeriq-card">
            {/* eBay Supply Intelligence (player-level) */}
            {result.ebaySupplySnapshot && (
              <div className="playeriq-ebay-supply">
      ...
      {result.warnings.length > 0 && (
        <div className="playeriq-warnings">
          <strong>Warnings:</strong> {result.warnings.join(", ")}
        </div>
      )}
      {result.nextActions.length > 0 && (
        <div className="playeriq-next-actions">
...
export default PlayerIQResultCard;
