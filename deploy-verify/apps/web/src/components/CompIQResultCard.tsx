...
import type { CompIQResponse } from "../types/compiq";
import "./CompIQResultCard.css";

interface Props {
  result: CompIQResponse;
}

const CompIQResultCard: React.FC<Props> = ({ result }) => {
  // Helper for supply signal color
  function supplySignalClass(signal?: string | null) {
    if (!signal) return "supply-unknown";
    if (signal === "Tightening") return "supply-tightening";
    if (signal === "Expanding" || signal === "Flooded") return "supply-expanding";
    if (signal === "Stable") return "supply-stable";
    return "supply-unknown";
  }

  return (
    <div className="compiq-card">
            {/* eBay Supply Intelligence */}
            {result.ebaySupply && (
              <div className="compiq-ebay-supply">
      ...
      {result.warnings.length > 0 && (
        <div className="compiq-warnings">
          <strong>Warnings:</strong> {result.warnings.join(", ")}
        </div>
      )}
      {result.nextActions.length > 0 && (
        <div className="compiq-next-actions">
...
export default CompIQResultCard;
