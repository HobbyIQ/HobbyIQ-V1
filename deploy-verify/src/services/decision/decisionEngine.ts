export function makeDecision({ payload, weighted, trends, supply, liquidityScore, absorptionRate, playerSignal, newsSignal, confidence, parallelInfo, comps, volatility, velocity, liquidityTier, usedInterpolation, marketImpact }: any) {
  ...
  const recentComps = comps.slice(-1).map((c: any) => ({
    date: c.date,
    price: c.price,
    grade: c.grade,
