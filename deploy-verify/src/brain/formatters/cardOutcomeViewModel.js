...
exports.formatCardOutcomeViewModel = formatCardOutcomeViewModel;
function formatCardOutcomeViewModel(payload, result) {
    ...
    const scenarios = (result.scenarios || []).map((s) => ({
        ...
        reasoning: (s.reasoning || []).map((r) => r.replace('Event:', 'If').replace('impact:', 'impact score'))
    }));
    return {
        success: true,
        summary: {
