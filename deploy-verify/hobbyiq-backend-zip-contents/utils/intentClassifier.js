...
function parseQuery(query) {
    ...
    if (parallelMatch)
        result.parallel = parallelMatch[1].toLowerCase();
    // Auto/non-auto
    if (/\bauto\b/i.test(query) && !/non[- ]?auto/i.test(query))
        result.auto = true;
    if (/non[- ]?auto/i.test(query))
        result.auto = false;
    ...
    // Compare phrasing
    if (/(compare|vs\.?|versus|better than|difference|head to head|which is better)/i.test(query))
        result.compare = true;
