...
function getVerdictLabel(recommendation: string) {
...
function getVerdictColor(recommendation: string) {
  switch (recommendation) {
...
function getShortAdvice(recommendation: string, summary: any) {
  switch (recommendation) {
...
function getRiskSummary(riskLevel: string) {
  switch (riskLevel) {
...
function formatCardDecisionViewModel(raw: any) {
  const summary = raw.summary || {};
  const zones = raw.zones || {};
  const insights = raw.insights || {};
  const reasoning = (raw.reasoning || []).map((r: string) =>
    r.replace('comps', 'recent sales').replace('Player/news signals', 'Player and news signals')
  );
...
export { formatCardDecisionViewModel };
