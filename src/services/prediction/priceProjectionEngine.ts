...
const BASE_MULTIPLIERS: Record<string, [number, number]> = {
  promotion: [1.3, 2.0],
...
export function projectPrice(payload: any, events: CardEvent[]) {
  ...
  let supplyAdj = 1;
  if (payload.supplyTrend2W && payload.supplyTrend2W < 0) supplyAdj += 0.05;
  if (payload.supplyTrend2W && payload.supplyTrend2W > 0) supplyAdj -= 0.05;
  let trendAdj = 1;
  if (payload.trendDirection === 'up') trendAdj += 0.07;
  if (payload.trendDirection === 'down') trendAdj -= 0.07;
  ...
  const probability = getProbability(payload, events, eventConfidence);
