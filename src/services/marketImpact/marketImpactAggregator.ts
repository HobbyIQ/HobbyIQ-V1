...
import { MarketImpactSignal, AggregatedMarketImpact } from './types';

export function aggregateMarketImpact(signals: MarketImpactSignal[]): AggregatedMarketImpact {
  ...
  signals.forEach(s => {
    if (s.direction === 'positive') pos += s.score * s.impactWeight;
    if (s.direction === 'negative') neg += s.score * s.impactWeight;
    score += s.score * s.impactWeight;
  });
  let overallDirection: 'positive' | 'negative' | 'neutral' = 'neutral';
  let pricePressure: 'upward' | 'downward' | 'neutral' = 'neutral';
  if (pos > neg && pos > 0) {
